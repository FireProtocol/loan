...
/**
 * @notice FireGovernance
 * @author Fire
 */
contract FireGovernance {
    string public constant name = "FIRE PROTOCOL";
    function passVotes() public pure returns (uint) {return 500000e8;}
    function passMantissa() public pure returns (uint) {return 0.2e8;}
    function proposalThreshold() public pure returns (uint) { return 500000e8; }
    function proposalMaxOperations() public pure returns (uint) { return 10; }
    function votingDelay() public pure returns (uint) { return 1; }
    function votingPeriod() public pure returns (uint) { return 86400; }
    ...
    function proposeLockup(uint _number) public {
        require(_number > 0, "FireGovernance::proposeLockup number must be greater than 0");
    ...
    function proposeFreed(address payable _account, uint _proposalId) public {
        require(state(_proposalId) != ProposalState.Active, "FireGovernance::proposeFreed voting is not closed");
        Proposal storage proposal = proposals[_proposalId];
        require(_account == msg.sender && _account == proposal.proposer, "FireGovernance::proposeFreed no permission to operate");
        require(checkPointProposal[_account] > 0, "FireGovernance::proposeFreed insufficient coins");
    ...
    function votesLockup(uint _number, uint _proposalId) public {
        require(_number > 0, "FireGovernance::votesLockup number must be greater than 0");
        require(state(_proposalId) == ProposalState.Active, "FireGovernance::votesLockup voting is closed");
        uint actualAmount = doTransferIn(msg.sender, _number);
    ...
    function votesFreed(address payable _account, uint _proposalId) public {
        require(state(_proposalId) != ProposalState.Active, "FireGovernance::votesFreed voting is not closed");
        require(msg.sender == _account, "FireGovernance::votesFreed no permission to operate");
    ...
    function propose(address[] memory _targets, uint[] memory _values, string[] memory _signatures, bytes[] memory _calldatas, string memory _description) public returns (uint) {
        require(checkPointProposal[msg.sender] >= proposalThreshold(), "FireGovernance::propose: proposer votes below proposal threshold");
        require(_targets.length == _values.length && _targets.length == _signatures.length && _targets.length == _calldatas.length, "FireGovernance::propose: proposal function information arity mismatch");
        require(_targets.length != 0, "FireGovernance::propose: must provide actions");
        require(_targets.length <= proposalMaxOperations(), "FireGovernance::propose: too many actions");

        uint latestProposalId = latestProposalIds[msg.sender];
        if (latestProposalId != 0) {
          ProposalState proposersLatestProposalState = state(latestProposalId);
          require(proposersLatestProposalState != ProposalState.Active, "FireGovernance::propose: one live proposal per proposer, found an already active proposal");
          require(proposersLatestProposalState != ProposalState.Pending, "FireGovernance::propose: one live proposal per proposer, found an already pending proposal");
        }
    ...
    function queue(uint _proposalId) public {
        require(state(_proposalId) == ProposalState.Succeeded, "FireGovernance::queue: proposal can only be queued if it is succeeded");
        Proposal storage proposal = proposals[_proposalId];
    ...
    function _queueOrRevert(address _target, uint _value, string memory _signature, bytes memory _data, uint _eta) internal {
        require(!timelock.queuedTransactions(keccak256(abi.encode(_target, _value, _signature, _data, _eta))), "FireGovernance::_queueOrRevert: proposal action already queued at eta");
        timelock.queueTransaction(_target, _value, _signature, _data, _eta);
    }

    function execute(uint _proposalId) public payable {
        require(state(_proposalId) == ProposalState.Queued, "FireGovernance::execute: proposal can only be executed if it is queued");
        Proposal storage proposal = proposals[_proposalId];
    ...
    function cancel(uint _proposalId) public {
        ...
        require(state != ProposalState.Executed, "FireGovernance::cancel: cannot cancel executed proposal");

        Proposal storage proposal = proposals[_proposalId];
        require(msg.sender == guardian || checkPointProposal[proposal.proposer] < proposalThreshold(), "FireGovernance::cancel: proposer above threshold");
    ...
    function state(uint _proposalId) public view returns (ProposalState) {
        require(proposalCount >= _proposalId && _proposalId > 0, "FireGovernance::state: invalid proposal id");
        Proposal storage proposal = proposals[_proposalId];
    ...
    function castVote(uint _proposalId, bool _support) public {
        require(getPriorVotes(msg.sender, _proposalId) > 0, "FireGovernance::castVote not enough votes");
    ...
    function castVoteBySig(uint _proposalId, bool _support, uint8 _v, bytes32 _r, bytes32 _s) public {
        ...
        require(signatory != address(0), "FireGovernance::castVoteBySig: invalid signature");
        return _castVote(signatory, _proposalId, _support);
    }

    function _castVote(address _voter, uint _proposalId, bool _support) internal {
        require(state(_proposalId) == ProposalState.Active, "FireGovernance::_castVote: voting is closed");
        Proposal storage proposal = proposals[_proposalId];
        Receipt storage receipt = proposal.receipts[_voter];
        require(receipt.hasVoted == false, "FireGovernance::_castVote: voter already voted");
    ...
    function underlying() internal pure returns (address) {
        return address(0x97814CF1e3b43Eb8b23F000dA3cb81CB405acD25);
    }

    function __acceptAdmin() public {
        require(msg.sender == guardian, "FireGovernance::__acceptAdmin: sender must be gov guardian");
        timelock.acceptAdmin();
    }

    function __abdicate() public {
        require(msg.sender == guardian, "FireGovernance::__abdicate: sender must be gov guardian");
        guardian = address(0);
    }

    function __queueSetTimelockPendingAdmin(address _newPendingAdmin, uint _eta) public {
        require(msg.sender == guardian, "FireGovernance::__queueSetTimelockPendingAdmin: sender must be gov guardian");
    ...
    function __executeSetTimelockPendingAdmin(address _newPendingAdmin, uint _eta) public {
        require(msg.sender == guardian, "FireGovernance::__executeSetTimelockPendingAdmin: sender must be gov guardian");
